...
program datatypes
    ...
    if (myRank .eq. 0) then
        ...
        do i = 1, matrix_dim
            call MPI_SEND(matrix(i,1), 1, vectortype, 1, tag, MPI_COMM_WORLD, ierr)
        end do

        call MPI_TYPE_FREE(vectortype, ierr)
    else
        call MPI_TYPE_CONTIGUOUS(matrix_dim, MPI_INTEGER, contigtype, ierr)
        call MPI_TYPE_COMMIT(contigtype, ierr)

        call print_typeinfo(contigtype, "contiguous")

        do i = 1, matrix_dim
            call MPI_RECV(matrix(1,i), 1, contigtype, 0, tag, MPI_COMM_WORLD, status, ierr)
        end do

        call print_matrix(matrix, matrix_dim, myRank)

        call MPI_TYPE_FREE(contigtype, ierr)
    endif
